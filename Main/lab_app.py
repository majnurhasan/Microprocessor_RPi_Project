...
@app.route("/proj_env_db")
def lab_env_db():
	conn=sqlite3.connect('/var/www/lab_app/proj_app.db')
	curs=conn.cursor()
	curs.execute("SELECT * FROM mosquitoReadings")
	mosquitoReadings = curs.fetchall()
	curs.execute("SELECT * FROM dailyReadings")
	dailyReadings = curs.fetchall()
	conn.close()
	return render_template("proj_env_db.html",mosReadings=mosquitoReadings,daiReadings=dailyReadings)
